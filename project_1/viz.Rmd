...
world_stats <- lapply(world_stats[,c(3:dim(world_stats)[2])], as.numeric.factor)
world_stats <- as.data.frame(world_stats)
...
p + geom_point(aes(colour = factor(ci_rating)), size = 4) +
  ...
  geom_text_repel(
    data = subset(world_stats_2018, Country.Name %in% c("United States of America","Canada","China","Brazil","France","Italy","India","Denmark","Russian Federation")), 
  ...
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  labs(title="Infant Mortality Rate vs Corruption in Countries", subtitle = "Data collected from World Bank and Corruption Perception Index") +
  labs(x ="Corruption Perception Index", y = "Infant mortality Rate (in 1000 live births)") + 
