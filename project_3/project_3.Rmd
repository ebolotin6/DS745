...
For the month of April 2018, there were `num_articles` published by the NYT (each with distinct headlines). **Examples** of headlines include:

```{r}
head(articles[articles$headline != "Unknown",])
```

All `num_articles` above belong to `num_topics` distinct topics. The `nrow(topic_tally[topic_tally$count>10,])` most frequent are plotted:
...
There are a total of `total_comments` for April 2018.
...
We now have `num_rows` comments to train on.
...
The histogram shows a highly right-skewed distribution. We can see that length bottoms out near 300 words per comment (sequence). However, the 95% quantile is equal to `quantiles[2]` words per comment. So we can use this length as the input_size.

**Next**: Set a max sequence length to `quantiles[2]` words and pad vectors less than this with zeros.
...
Below is the neural network architecture. We are using a sequential model with 4 fully connected layers, 1 pooling layer, and 2 dropout layers. Notice the use of the embedding layer to represent vectorized words in compact form (input dim = `vocab_len` and output = 300).
...
This architecture has a large amount of weights: 24.7M, which is overly complex for the task at hand. I selected this architecture to demonstrate an important training principle (regarding text classification) with neural networks, which I discuss in the conclusion section.
...
As we see from the evaluation of the model on the test data, the model produces **~63%** classification accuracy on the test set (`nrow(x_test)` observations and `num_classes` classes). This is not a good result.

The training plot is highly indicative of what is happening here. From the training plot we can see that model begins to overfit the training set at the 5th epoch. By the 10th epoch, validation accuracy has peaked at ~63%, while training accuracy peaks at the 13th epoch at 81%.

Why does the model have poor prediction accuracy? This is due to the generic nature of comments on NYT articles and the topics created for them. Many comments are simply not characteristic of any singular topic. Take for example this subset of comments with a *newsDesk* title of 'Sports' and *sectionName* of 'Pro Football':

```{r}
slice(comments[comments$newDesk == "Sports" & comments$sectionName == "Pro Football",], 50:60)
```

The conversation in these comments is about harassment, lawsuits, and discrimination against cheerleaders. There is no actual conversation about sports, athletes, or NFL games. The real classification topic here is *politics*. Such is also the case with the comments from NYT April 2017 regarding Colin Kaepernick. When Kaepernick created a political movement by taking a knee during the national anthem, all of the comments in response to the subsequent NYT article had to do with the political repercussions as well as Donald Trump.

So the neural network has a difficult time distinguishing comments that could truly belong to multiple classes. It cannot accurately classify such comments **given this amount of data**.
