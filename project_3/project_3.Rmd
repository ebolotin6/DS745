...
pacman::p_load(keras, ggplot2, dplyr)
```

## Dataset description

The dataset that I have chosen to use for this analysis is from the New York Times. More specifically, this open-source dataset contains NYT articles and reader comments from January to May 2017 and January to April 2018. 
As mentioned in the official dataset description [1], the comments' data contains "over 2 million comments and 34 features", while the articles' data contains more than 9000 articles 16 features.

## Objective

The objective of this project is to analyze the comments of NYT readers with the goal of classifying comments into distinct article topics. Article topics are defined by editors of the NYT in column entitled "newDesk", as we will review below.

## Methodology: steps detailing how the analysis was carried out

The methodology used in this project to perform text classification is a form of supervised learning. Namely, we will be training a neural network (multiple layer perceptron) on the comments data to predict article topic.
...
comments <- as.data.frame(comments[,c("commentBody","newDesk")])
